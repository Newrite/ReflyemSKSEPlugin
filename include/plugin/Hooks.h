...
public:
  static auto install_hook(SKSE::Trampoline &trampoline) -> void {
    ...
    _process_attack =
        trampoline.write_call<5>(hook.address() + RELOCATION_OFFSET(Offsets::on_attack_data, 0), process_attack);
...
public:
  static auto install_hook(SKSE::Trampoline &trampoline) -> void {
    ...
    _attack_action =
        trampoline.write_call<5>(hook.address() + RELOCATION_OFFSET(Offsets::on_attack_action, 0), attack_action);
...
public:
  static auto install_hook(SKSE::Trampoline &trampoline) -> void {
    ...
    _dual_modify_actor_value_second_inner_call = trampoline.write_call<5>(
        hook.address() + RELOCATION_OFFSET(Offsets::on_dual_modify_actor_value_second_inner_call, 0),
...
public:
  static auto install_hook(SKSE::Trampoline &trampoline) -> void {
    ...
    _adjust_active_effect = trampoline.write_call<5>(
        hook.address() + RELOCATION_OFFSET(Offsets::on_adjust_active_effect, 0), adjust_active_effect);
...
public:
  static auto install_hook(SKSE::Trampoline &trampoline) -> void {
    ...
    _main_update =
        trampoline.write_call<5>(hook.address() + RELOCATION_OFFSET(Offsets::on_main_update, 0), main_update);
