...
struct ResourceDrain final
{
  ...
  auto drain(RE::Actor& actor) -> void
  {
    ...
    if (health != 0.f) { spend_actor_value(actor, RE::ActorValue::kHealth, std::abs(health)); }
    if (magicka != 0.f) { spend_actor_value(actor, RE::ActorValue::kMagicka, std::abs(magicka)); }
    if (stamina != 0.f) { spend_actor_value(actor, RE::ActorValue::kStamina, std::abs(stamina)); }
  }
};
