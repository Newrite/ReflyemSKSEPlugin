...
{
  if (this_) {
    logi("Check equip state for: {}"sv, this_->GetName());
  }
  return 1;
}
                                           ...
                                           bool play_sound) -> void
{
  ...
  if (bound_object) {
    ...
    if (alchemy) {
      ...
      logger::info("Pick null alchemy: {}"sv, object->GetName());
    }
  }
  ...
  if (this_ && reason == RE::ITEM_REMOVE_REASON::kRemove && !move_to_ref && !drop_loc && !rotate && count > 0 && item) {
    ...
    if (config.potions_drink_limit().enable() && config.potions_drink_limit().no_remove_keyword()) {
      ...
      if (alchemy_item && alchemy_item->HasKeyword(config.potions_drink_limit().no_remove_keyword())) {
        ...
        logi("No remove alchemy item: {}"sv, alchemy_item->GetFullName());
    ...
    if (config.poison_rework().enable() && config.poison_rework().no_remove_keyword()) {
      ...
      if (alchemy_item && alchemy_item->HasKeyword(config.poison_rework().no_remove_keyword())) {
        ...
        logi("No remove poison alchemy item: {}"sv, alchemy_item->GetFullName());
...
auto OnSomeMenuEquipState::get_equip_state_01(RE::Actor* actor, RE::TESBoundObject* object, void* a3, void* a4)
    -> std::uint32_t
{
  if (object) {
    logi("01 Check state for: {}"sv, object->GetName());
  }
  return 1;
}

auto OnSomeMenuEquipState::get_equip_state_02(RE::Actor* actor, RE::TESBoundObject* object, void* a3, void* a4)
    -> std::uint32_t
{
  if (object) {
    logi("02 Check state for: {}"sv, object->GetName());
  }
  return 1;
}

auto OnSomeMenuEquipState::get_equip_state_03(RE::Actor* actor, RE::TESBoundObject* object, void* a3, void* a4)
    -> std::uint32_t
{
  if (object) {
    logi("03 Check state for: {}"sv, object->GetName());
  }
  return 1;
}

auto OnSpellEquip::equip_spell_left(RE::Actor* actor, RE::Actor::SlotTypes slot_type, RE::MagicItem* magic_item)
    -> void*
{
...
auto OnSomeEquip::some_equip(RE::ActorEquipManager* equip_manager,
                             RE::Actor* actor,
                             ...
                             void* extra_data_list) -> void
{
  ...
  if (!is_equipped && bound_object->IsWeapon() && bound_object->As<RE::TESObjectWEAP>()->IsOneHandedSword()) {
    is_equipped = true;
    logi("dual equip"sv);
...
auto OnSomeEquip::some_equip_default_equip(RE::ActorEquipManager* equip_manager,
                                           RE::Actor* actor,
...
auto OnEquipManagerEquipSpell::equip_spell(RE::ActorEquipManager* equip_manager,
                                           RE::Actor* actor,
                                           RE::TESBoundObject* bound_object,
