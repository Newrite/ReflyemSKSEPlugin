  ...
  if (config.cast_on_timing_block().enable())
    {
      Core::cast_on_handle_formlists(
          config.cast_on_timing_block().formlist_needkw(),
...
auto cast_on_block_parry(RE::Actor& target, RE::Actor& caster, const Config& config) -> void
{
  if (config.cast_on_block_parry().enable())
    {
      Core::cast_on_handle_formlists(
          config.cast_on_block_parry().formlist_needkw(),
...
{
  if (!config.timing_block().enable_sparks()) { return; }
  ...
  if (parry) { place_form_at_me(block_node, config.timing_block().spark_halo()); }
    ...
    const Config& config) -> bool
{
  ...
  if (!hit_data.flags.any(RE::HitData::Flag::kBlocked)) { return false; }

  auto& actors_cache = Core::ActorsCache::get_singleton();
  const auto& actor_data = actors_cache.get_or_add(target.formID).get();

  if (is_allow_timing_parry(attacker, target, actor_data, config)) { return true; }

  if (is_allow_timing_block(attacker, target, actor_data, config)) { return true; }
...
auto on_weapon_hit(RE::Actor& target, RE::HitData& hit_data, const Config& config) -> void
{
  ...
  if (!hit_data.flags.any(RE::HitData::Flag::kBlocked)) { return; }
