	...
	namespace MagicShield
	{
			...
			const Reflyem::Config& config) -> void
		{
			...
			auto cost_per_damage =
				Reflyem ::Core ::get_effects_magnitude_sum(effects).value_or(1.f);

			auto absorb_cost = magicka - (absorb_damage * cost_per_damage);
			logger::info("magicka: {}, percent: {}, absorb_cost: {}, absorb_damage: {}, cost_per_damage: {}", magicka, magic_shield_percent, absorb_cost, absorb_damage, cost_per_damage);
			if (absorb_cost >= 0.f) {
