...
auto magic_shield(RE::Actor& target, float& damage_value, const Reflyem::Config& config) -> void {
  ...
  auto effects         = Reflyem::Core::get_effects_by_keyword(target, *config.magic_shield_cost_keyword);
  auto cost_per_damage = Reflyem ::Core ::get_effects_magnitude_sum(effects).value_or(1.f);

  auto absorb_cost = magicka - (absorb_damage * cost_per_damage);
  logger::info("magicka: {}, percent: {}, absorb_cost: {}, absorb_damage: {}, cost_per_damage: {}", magicka,
