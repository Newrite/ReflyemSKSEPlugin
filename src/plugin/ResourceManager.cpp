...
namespace Reflyem
{
	namespace ResourceManager
	{
			...
			const Reflyem::Config& config) -> float
		{
			...
			auto cost =
				(weapon_weight * attack_cost_mult * infamy) + (weapon_weight * infamy * power_attack_cost_mult * config.resource_manager_power_attack_mult);
			...
			const Reflyem::Config& config) -> float
		{
			...
			auto cost =
				(weight * bash_cost_mult * infamy) + (weight * infamy * power_bash_cost_mult * config.resource_manager_power_attack_mult);
		...
		auto ranged_spend_handler() -> void
		{
			for (auto& [actor, drain_value] : rm_map) {
				if (actor && drain_value) {
					...
					auto isDraw = state == RE::ATTACK_STATE_ENUM::kBowDraw || state == RE::ATTACK_STATE_ENUM::kBowDrawn || state == RE::ATTACK_STATE_ENUM::kBowAttached;
