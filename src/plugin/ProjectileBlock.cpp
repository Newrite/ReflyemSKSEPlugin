    ...
    RE::Actor* caster) -> void
{
  ...
  if (!allow_cast && !Core::try_has_absolute_keyword(caster, keyword_ptr)) { return; }
...
auto cast_on_timing_block(RE::TESObjectREFR* target, RE::Actor* caster, const Config& config)
    -> void
{
  if (config.cast_on_projectile_timing_block().enable())
    {
      cast_on_handle_formlists(
          config.cast_on_projectile_timing_block().formlist_needkw(),
...
auto cast_on_block(RE::TESObjectREFR* target, RE::Actor* caster, const Config& config) -> void
{
  if (config.cast_on_projectile_block().enable())
    {
      cast_on_handle_formlists(
          config.cast_on_projectile_block().formlist_needkw(),
...
auto get_heading_angle_re(
    RE::TESObjectREFR* a_object,
    RE::TESObjectREFR* a_target,
    bool a_unk_flag,
    double& a_result) -> bool
{
...
bool is_in_block_angle(RE::Actor* blocker, RE::TESObjectREFR* a_obj)
{
  ...
  if (!settings || !a_obj || !blocker) { return false; }
...
auto is_projectile_spell_ignore_block(const RE::Projectile* projectile, const Config& config)
    -> bool
{
  if (!projectile) { return false; }
  let spell = projectile->spell;
  if (!spell) { return false; }
  if (!config.projectile_block().exclusive_keyword()) { return false; }
...
auto is_projectile_spell_ignore_timing_block(const RE::Projectile* projectile, const Config& config)
    -> bool
{
  if (!projectile) { return false; }
  let spell = projectile->spell;
  if (!spell) { return false; }
  if (!config.projectile_block().exclusive_timing_keyword()) { return false; }
  return spell->HasKeyword(config.projectile_block().exclusive_timing_keyword());
}

auto is_projectile_ammo_ignore_timing_block(const RE::Projectile* projectile, const Config& config)
    -> bool
{
  if (!projectile) { return false; }
  let ammo = projectile->ammoSource;
      ...
      if (ni_shooter)
        {
          ...
          if (shooter_object) { return shooter_object; }
        }
    }
  return nullptr;
}
...
auto get_shooter_from_projectile(const RE::Projectile* projectile) -> RE::Actor*
{
  ...
  if (shooter_ref) { return shooter_ref->As<RE::Actor>(); }
  return nullptr;
}
...
auto is_shooter_ignore_block(const RE::Projectile* projectile, const Config& config) -> bool
{
  ...
  if (shooter &&
      Core::try_has_absolute_keyword(shooter, config.projectile_block().exclusive_keyword()))
    {
      return true;
    }
  return false;
...
{
  if (!config.projectile_block().enable_timing_block()) { return false; }

  if (!blocker || !projectile) { return false; }

  if (!Core::try_has_absolute_keyword(
          blocker,
          config.projectile_block().enable_timing_block_keyword()))
    {
  ...
  if (projectile_type != ProjectileType::kArrow &&
      (is_projectile_spell_ignore_timing_block(projectile, config) ||
       !Core::try_has_absolute_keyword(
           blocker,
           config.projectile_block().enable_magick_timing_block_keyword())))
    {
  ...
  if (projectile_type == ProjectileType::kArrow &&
      (is_projectile_ammo_ignore_timing_block(projectile, config) ||
       !Core::try_has_absolute_keyword(
           blocker,
           config.projectile_block().enable_arrow_timing_block_keyword())))
    {
      logi("Ammo ignore timing block"sv);
      return false;
    }

  let timing_effects = Core::try_get_effects_by_keyword(
      blocker,
      config.projectile_block().timing_block_timer_keyword());
  let block_timing = config.projectile_block().block_timing() +
...
{
  if (!config.projectile_block().enable_sparks()) { return; }

  const auto block_node =
      TimingBlock::place_form_at_me(blocker, config.projectile_block().blank_activator());
  ...
  if (parry) { TimingBlock::place_form_at_me(block_node, config.projectile_block().spark_halo()); }
  block_node->SetDelete(true);
}

auto try_block_projectile(
    RE::Actor* blocker,
    ...
    const Config& config) -> bool
{
  ...
  if (right_hand) { is_shield = right_hand->IsArmor(); }
  ...
  if (is_allow_timing_block(blocker, projectile, projectile_type, actor_data, config))
    {
      ...
      if (projectile_type != ProjectileType::kFlame) { destroyProjectile(projectile); }
  ...
  if (!config.projectile_block().enable_block()) { return false; }
  ...
  if (projectile_type == ProjectileType::kArrow &&
      (is_projectile_ammo_ignore_block(projectile, config) ||
       !Core::try_has_absolute_keyword(
           blocker,
           config.projectile_block().enable_arrow_block_keyword())))
    {
      return false;
    }
  ...
  if (projectile_type != ProjectileType::kArrow &&
      (is_projectile_spell_ignore_block(projectile, config) ||
       !Core::try_has_absolute_keyword(
           blocker,
           config.projectile_block().enable_magick_block_keyword())))
    {
      return false;
    }
  ...
  if (projectile_type != ProjectileType::kArrow &&
      config.projectile_block().enable_block_magick_for_magick())
    {
  ...
  let cost_effects =
      Core::try_get_effects_by_keyword(blocker, config.projectile_block().block_cost_keyword());
  ...
  if (block_av > 200.f) { block_av = 200.f; }
  if (block_av < 0.f) { block_av = 1.f; }
...
auto process_projectile_block(
    RE::Actor* blocker,
    RE::Projectile* projectile,
    const ProjectileType projectile_type,
    const Config& config) -> bool
{
  if (!blocker || !projectile || is_shooter_ignore_block(projectile, config)) { return false; }
  if (!blocker->IsBlocking() || !is_in_block_angle(blocker, projectile)) { return false; }
...
// from dTry's Valhalla Combat https://github.com/D7ry/valhallaCombat/blob/d27c89bcdf32d38a579c428c46e1dcbb0a790add/src/bin/Hooks.cpp#L110C22-L110C22
          ...
          if (ref_a && ref_a->formType == RE::FormType::ActorCharacter)
            {
              ...
              return process_projectile_block(
                  ref_a->As<RE::Actor>(),
                  projectile,
                  projectile_type,
                  config);
