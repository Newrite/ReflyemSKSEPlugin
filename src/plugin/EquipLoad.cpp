...
auto eval_equip_load(const RE::Actor& actor) -> float
{
  ...
  const auto weapon_weight = [&]() -> float {
    auto weight = 0.f;
    if (right_equipped_weapon) {
      weight += right_equipped_weapon->weight;
    }
    if (left_equipped_weapon) {
      weight += left_equipped_weapon->weight;
    }
    return weight;
  }();
  ...
  return config.equip_load().equip_weight_mult() * ((armor_weight * config.equip_load().armor_weight_mult()) +
...
auto update_actor(RE::Actor& actor, const Config& config) -> void
{
  ...
  if (!config.equip_load().enable_for_npc() && !actor.IsPlayerRef()) {
    return;
  }
