...
{
  auto stamina_shield_percent =
      config.stamina_shield().enable_av() ? target.GetActorValue(config.stamina_shield().av()) : 0.f;
  stamina_shield_percent = stamina_shield_percent +
                           Core::get_effects_magnitude_sum(
                               Core::try_get_effects_by_keyword(&target, config.stamina_shield().keyword_shield_value()))
                               .value_or(0.f);

  if (stamina_shield_percent <= 0.f) {
    return;
  }
  if (stamina_shield_percent > 100.f) {
    stamina_shield_percent = 100.f;
  }
  ...
  const auto effects = Core::get_effects_by_keyword(target, *config.stamina_shield().mgef_cost_keyword());
  ...
  logger::debug("stamina: {}, percent: {}, absorb_cost: {}, absorb_damage: {}, "
                "cost_per_damage: {}"sv,
                stamina,
...
{
  if (Core::can_modify_actor_value(this_, actor, value, av) && allow_magic_shield_effect(*this_, config)) {
