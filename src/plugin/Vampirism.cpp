...
auto modify_actor_value(RE::ValueModifierEffect* a_this, RE::Actor* a_actor, float& a_value, RE::ActorValue av,
  ...
  if (Reflyem::Core::can_modify_actor_value(a_this, a_actor, a_value, av)) {
    ...
    if (config.magic_vampirism_mgef_health_enable) {
      mgef_vampirism(*a_actor, *agressor, a_value, *config.magic_vampirism_mgef_health_keyword,
                     RE::ActorValue::kHealth);
    }

    if (config.magic_vampirism_mgef_stamina_enable) {
      mgef_vampirism(*a_actor, *agressor, a_value, *config.magic_vampirism_mgef_stamina_keyword,
                     RE::ActorValue::kStamina);
    }

    if (config.magic_vampirism_mgef_magicka_enable) {
      mgef_vampirism(*a_actor, *agressor, a_value, *config.magic_vampirism_mgef_magicka_keyword,
...
auto on_weapon_hit(RE::Actor* target, RE::HitData& hit_data, const Reflyem::Config& config) -> void {
  ...
  if (config.vampirism_mgef_health_enable) {
    mgef_vampirism(*target, *agressor, hit_data.totalDamage, *config.vampirism_mgef_health_keyword,
                   RE::ActorValue::kHealth);
  }

  if (config.vampirism_mgef_stamina_enable) {
    mgef_vampirism(*target, *agressor, hit_data.totalDamage, *config.vampirism_mgef_stamina_keyword,
                   RE::ActorValue::kStamina);
  }

  if (config.vampirism_mgef_magicka_enable) {
    mgef_vampirism(*target, *agressor, hit_data.totalDamage, *config.vampirism_mgef_magicka_keyword,
                   RE::ActorValue::kMagicka);
  }
}
} // namespace Vampirism
