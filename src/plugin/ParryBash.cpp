...
auto is_allow_parry_bash(RE::Actor* attacker, RE::Actor* target, const Config& config) -> bool
{
  ...
  const auto timing_effects = Core::try_get_effects_by_keyword(attacker, config.parry_bash().parry_timing_keyword());
  const auto parry_timing =
      config.parry_bash().parry_timing() + Core::get_effects_magnitude_sum(timing_effects).value_or(0.f);

  const auto diff = 1.f - target_data.bash_parry_timer();

  logger::debug("{} -> {}: diff {} parry_timing {}"sv,
                attacker->GetDisplayFullName(),
                target->GetDisplayFullName(),
                diff,
                parry_timing);
  if (diff > parry_timing) {
    logger::debug("{} -> {}: ignore bash hit"sv, attacker->GetDisplayFullName(), target->GetDisplayFullName());
    return false;
  }
  logger::debug("{} -> {}: parry bash"sv, attacker->GetDisplayFullName(), target->GetDisplayFullName());
  if (Core::ExternalApi::true_hud_api) {
    Core::ExternalApi::true_hud_api->FlashActorSpecialBar(SKSE::GetPluginHandle(), attacker->GetHandle(), true);
  }
  return true;
}
...
auto cast_on_parry_bash(RE::Actor& target, RE::Actor& caster, const Config& config) -> void
{
  if (config.cast_on_parry_bash().enable()) {
    Core::cast_on_handle_formlists(
        config.cast_on_parry_bash().formlist_needkw(), config.cast_on_parry_bash().formlist_spells(), caster, target);
  }
}
...
auto precision_pre_hit_callback(const PRECISION_API::PrecisionHitData& hit_data) -> PRECISION_API::PreHitCallbackReturn
{
  ...
  if (hit_data.target->As<RE::Actor>() && config.parry_bash().enable_vanilla_bash_ranged() &&
      ...
      (Core::is_casting_actor(*hit_data.target->As<RE::Actor>()) ||
       Core::is_bow_or_crossbow_attack_actor(*hit_data.target->As<RE::Actor>()))) {
    return result;
  }

  const auto& attacker_data = Core::ActorsCache::get_singleton().get_or_add(hit_data.attacker->formID).get();
  ...
  if (is_allow_parry_bash(hit_data.attacker, target, config)) {
    ...
    if (hit_data.attacker && Core::is_bashing(hit_data.attacker)) {
      result.bIgnoreHit = true;
    }
  }
    ...
    -> PRECISION_API::WeaponCollisionCallbackReturn
{
  ...
  const auto& config = Config::get_singleton();
  if (!hit_data.target || !hit_data.attacker || !config.parry_bash().enable() ||
      !config.parry_bash().enable_precision_weapon_collide()) {
    return result;
  }
