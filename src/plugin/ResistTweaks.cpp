...
// TODO: Дополнить логику новых резистов для эффектов большей вариативностью в конфиге
auto check_resistance(RE::MagicTarget&          this_, RE::MagicItem& magic_item,
  ...
  if (alchemy_item &&
      ((!alchemy_item->IsPoison() && !effect.IsHostile()) || (alchemy_item->GetSpellType() ==
                                                              RE::MagicSystem::SpellType::kIngredient
                                                              && alchemy_item->IsFood()))) {
  ...
  if (resist_value > 800.f) {
    return 0.2f;
  }
  if (resist_value < 0.f) {
    return 1.f;
  }
  const auto resist_percent = 1.f - (resist_value * 0.001f);

  logger::debug("ResistAV: {} ResistPercent: {}"sv, static_cast<int32_t>(resist_av),
                resist_percent);
