...
auto check_resistance(RE::MagicTarget& this_, RE::MagicItem& magic_item, const RE::Effect& effect,
  ...
  const auto resist_value_after_penetration = [config](RE::Actor*           value_owner,
                                                       const RE::ActorValue resist_av) -> float {
    const auto resist_value = value_owner->GetActorValue(resist_av);

    if (!config.resist_tweaks().enable_resist_penetration()) {
      return resist_value;
    }
  ...
  if (magic_item.hostileCount <= 0 ||
      bound_object && bound_object->formType == RE::FormType::Armor) {
  ...
  auto second_resist_percent = [&](RE::Actor* value_owner, const RE::MagicItem& item) -> float {
    ...
    if (item.IsPoison()) {
      if (config.resist_tweaks().no_double_resist_check_poison()) {
        return 1.f;
      }
    ...
    if (resist_av == second_resist_av) {
      return 1.f;
    }

    const auto second_resist_value = resist_value_after_penetration(value_owner, second_resist_av);
    if (second_resist_value <= low_cap) {
      return 1.f;
    }
    if (second_resist_value >= high_cap) {
      return max_resist;
    }
  ...
  if (config.resist_tweaks().enable_damage_resist_tweak() &&
    ...
    if (config.resist_tweaks().no_double_damage_resist_check()) {
    ...
    if (resist_value >= damage_resist_high_cap) {
      return max_resist * second_resist_percent;
    }
