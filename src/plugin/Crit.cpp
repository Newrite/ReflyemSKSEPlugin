...
auto crit(RE::Actor& aggressor, float& damage, const RE::ActorValue crit_damage_av, const std::int32_t crit_damage_high)
    -> void
{
...
auto on_weapon_hit(RE::Actor* target, RE::HitData& hit_data, const Config& config) -> void
{
  ...
  if (!aggressor || !aggressor.get() || target->IsDead() || Core::try_has_absolute_keyword(target, config.weapon_crit().keyword_immun())) {
    return;
  }

  if (is_critical(*aggressor, config.weapon_crit().chance_av())) {

    if (config.weapon_crit().crit_sound()) {
      Core::play_sound(config.weapon_crit().crit_sound(), aggressor.get());
    }
    
    if (config.cast_on_crit().enable() && config.cast_on_crit().enable_weapon() && hit_data.weapon) {
                        ...
                        const Config& config) -> void
{
  ...
  if (Core::can_modify_actor_value(this_, actor, value, av) && allow_magic_crit(*this_, config, *actor)) {
    ...
    if (is_critical(*caster, config.magick_crit().chance_av())) {
      ...
      if (config.magick_crit().crit_sound()) {
        Core::play_sound(config.weapon_crit().crit_sound(), caster);
      }
