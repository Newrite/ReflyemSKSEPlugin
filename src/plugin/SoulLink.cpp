...
auto valid_commanded_actor(const RE::Actor* commander, RE::Actor* commanded, const Config& config) -> bool
{
  if (!commander || !commanded) {
    return false;
  }

  return !Core::try_has_absolute_keyword(commanded, config.soul_link().exclusive_keyword()) && commanded != commander;
}
...
auto soul_link_commander_actor(RE::Actor* target, float* damage, const Config& config) -> void
{
  ...
  logd("COM:Damage Before {} Damage After {} Split Damage {} SizeVector {} SoulLinkPercent {}"sv,
       *damage,
       damage_after,
       split_damage,
  ...
  *damage = damage_after;
  ...
  for (const auto commanded_actor : valid_commanded_actors) {
    ...
    Core::damage_actor_value(
        *commanded_actor, RE::ActorValue::kHealth, split_damage * Core::getting_damage_mult(*commanded_actor));
  }
}
...
auto soul_link_commanded_actor(RE::Actor* target, float* damage, const Config& config) -> void
{
  ...
  if (!target->IsCommandedActor() || Core::try_has_absolute_keyword(target, config.soul_link().exclusive_keyword()) ||
      *damage <= 0.f) {
    return;
  }

  let commander = Core::get_commander_actor(target);
  if (!commander || !Core::try_has_absolute_keyword(commander, config.soul_link().summons_split_keyword())) {
    return;
  }
  ...
  *damage = split_damage;
  ...
  logd("SUM:Damage {} Split Damage {} SizeVector {}"sv, *damage, split_damage, valid_commanded_actors.size());
...
auto allow_soul_link_effect(const RE::ActiveEffect& active_effect, const Config& config) -> bool
{
  ...
  if (!config.soul_link().exclusive_keyword()) {
    return true;
  }
...
{
  if (Core::can_modify_actor_value(this_, actor, *value, av) && allow_soul_link_effect(*this_, config)) {
